...
export const detectAnomaly = (historyRows, features) => {
    ...
    features.forEach(f => {
        ...
        // We use 2.5 for "Moderate/High" glitch detection
...
export const identifyArchetype = (rows, trackables) => {
    ...
    if (stats.daily_mantra > 0.7 && stats.rotting_time < 0.2) return "PRODUCTIVE_MONK";

    // 2. THE ROTTER: High Rotting, Low Grass, Low Mood
    if (stats.rotting_time > 0.6 && stats.moodScore < 0.4) return "VOID_DWELLER";

    // 3. THE SOCIALITE: High Social/Yap sessions (if they exist) or high energy
    if (stats.moodScore > 0.7 && stats.gym_log > 0.5) return "VIBE_MAXIMIZER";
