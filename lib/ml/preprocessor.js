...
function imputeMissingDays(rows, trackables) {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        // Force date floor to prevent millisecond jitter from creating "phantom" gaps
        const d1 = new Date(prev.date).setHours(0, 0, 0, 0);
        const d2 = new Date(curr.date).setHours(0, 0, 0, 0);
...
function getPearsonCorrelation(x, y) {
    ...
    // EPSILON_GUARD: If denominator is near zero, it's a stable pattern. 
    // Return the fallback immediately to prevent precision jitters.
