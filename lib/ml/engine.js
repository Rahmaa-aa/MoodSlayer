...
export const trainPredictor = async (data, targetKey, modelId = 'default-mood-model') => {
    ...
    // 6. Calculate Confidence with Squared SNR (Signal-to-Noise Ratio) Dampener
    // We calculate Pearson correlations to see if there's any actual underlying signal
    const { calculateCorrelations } = await import('./preprocessor');
    const corrs = calculateCorrelations(data, targetKey);
    const maxCorr = corrs.length > 0 ? Math.max(...corrs.map(c => c.strength)) : 0;

    // Aggressive Dampener: Signal must be significant (> 0.2) and we use a power function
    // to slam confidence for noise, but reward clear patterns (>= 0.5)
    const threshold = 0.5;
    const snrDampener = Math.pow(Math.max(0, maxCorr) / threshold, 2);

    const dataVolumeCoeff = Math.min(1, rows.length / 30);
    const accuracyCoeff = Math.max(0, 1 - (finalLoss * 2));

    // Confidence is now a product of Volume, Accuracy, and Squared Signal Strength
    const confidence = ((dataVolumeCoeff * 0.4) + (accuracyCoeff * 0.6)) * Math.min(1, snrDampener);
