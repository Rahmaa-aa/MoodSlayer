...
export const calculateStreak = (entries, volitionShield = false) => {
    ...
    if (lastEntryDate !== todayStr && lastEntryDate !== yesterdayStr) {
        // If Volition Shield is active, we don't return 0 immediately. 
        // We look for the last entry and resume from there, effectively "pausing" the streak gap.
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (diffDays === 1 || (volitionShield && diffDays > 1)) {
            // In Volition Shield mode, gaps are ignored—we just count consecutive "logged days"
            streak++;
