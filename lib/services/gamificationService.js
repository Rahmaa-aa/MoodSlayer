...
export const calculateStreak = (entries, volitionShield = false) => {
    if (!entries || entries.length === 0) return 0;

    // 1. Filter out "Illegal Backfills" unless Volition Shield is active.
    // An illegal backfill is an entry created > 48 hours after its target date.
    const filteredEntries = entries.filter(entry => {
        if (volitionShield) return true; // Survival mode users are allowed to backfill
        if (!entry.createdAt) return true; // Legacy entry fallback

        const loggedAt = new Date(entry.createdAt);
        const intendedDate = new Date(entry.date);
        const hoursDiff = Math.abs(loggedAt - intendedDate) / (1000 * 60 * 60);

        return hoursDiff <= 48; // Must log within 48h to count for streak
    });
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (diffDays === 1 || (volitionShield && diffDays > 1)) {
            streak++;
            currentDateStr = prevEntryDateStr;
        } else {
            break; // Gap found and shielded is off
        }
    }
...
export const calculateRPGStats = (entries, goals = [], globalSurvivalMode = false) => {
    if (!entries) return {};
    ...
    entries.forEach(entry => {
        ...
        const isEntrySurvival = data.survivalMode || globalSurvivalMode;

        // 1. SURVIVAL XP (Automatic detection)
        SURVIVAL_HABITS.forEach(h => {
            if (data[h] === true || (typeof data[h] === 'number' && data[h] > 0)) {
                // Boost XP in Survival Mode
                stats.SURVIVAL.xp += isEntrySurvival ? 15 : 5;
            }
        });
        ...
        habits.forEach(h => {
            ...
            goalsProgress.forEach(goal => {
                if (goal.linkedHabits.includes(h)) {
                    ...
                    if (isMet) {
                        let xpGain = (typeof val === 'number' && typeof condition?.value === 'number')
                            ? Math.min(Math.abs(val), 15)
                            : 10;

                        // Survival boost for goals linked to survival habits
                        if (isEntrySurvival && SURVIVAL_HABITS.includes(h)) {
                            xpGain *= 2;
                        }
