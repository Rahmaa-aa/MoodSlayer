...
import { Zap, Brain, Sparkles, TrendingUp, TrendingDown, Target, HelpCircle, CheckSquare, Square, Settings, Eye, EyeOff, Activity, ChevronUp, ChevronDown, ListFilter, Calendar as CalendarIcon, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function CyclesPage() {
    ...
    useEffect(() => {
        ...
        if (savedSettings) {
            ...
            if (parsed.visibleWidgets) setVisibleWidgets(prev => ({ ...prev, ...parsed.visibleWidgets }))
            if (parsed.collapsedWidgets) setCollapsedWidgets(prev => ({ ...prev, ...parsed.collapsedWidgets }))
    ...
    useEffect(() => {
        if (mounted) {
            localStorage.setItem('mood_cycles_settings', JSON.stringify({ visibleWidgets, collapsedWidgets, chartType }))
        }
    }, [visibleWidgets, collapsedWidgets, chartType, mounted])
    ...
    return (
        <div className="page-container">
            ...
            <section className="cyber-card" style={{ padding: 0, borderBottom: '3px solid black' }}>
                <div
                    onClick={() => setCollapsedWidgets(prev => ({ ...prev, calendar: !prev.calendar }))}
                    className="cyber-header clickable"
                    style={{
                        background: 'black',
                        color: 'white',
                        margin: 0,
                ...
                >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <CalendarIcon size={16} /> NEURAL_HISTORY_GRID
                    </div>
                    {collapsedWidgets.calendar ? <ChevronDown size={18} /> : <ChevronUp size={18} />}
                </div>
            ...
            <div style={{
                display: 'grid',
                gridTemplateColumns: collapsedWidgets.predictors ? '40px 1.5fr minmax(280px, 0.8fr)' : 'minmax(300px, 1fr) 1.5fr minmax(280px, 0.8fr)',
                gap: '20px',
                alignItems: 'start',
                ...
                {visibleWidgets.predictors && (
                    <div style={{ position: 'relative' }}>
                        {/* Panel Toggle Handle */}
                        <button
                            onClick={() => setCollapsedWidgets(prev => ({ ...prev, predictors: !prev.predictors }))}
                            style={{
                                position: 'absolute',
                                top: '50px',
                                right: '-16px',
                        ...
                        >
                            {collapsedWidgets.predictors ? <ChevronRight size={16} strokeWidth={3} /> : <ChevronLeft size={16} strokeWidth={3} />}
                        </button>

                        <section className="cyber-card" style={{ display: collapsedWidgets.predictors ? 'none' : 'block', height: '100%', padding: 0, overflow: 'hidden' }}>
                            <div
                                onClick={() => setCollapsedWidgets(prev => ({ ...prev, predictors: !prev.predictors }))}
                                className="cyber-header clickable"
                                style={{
                            ...
                            >
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <ListFilter size={14} /> {!collapsedWidgets.predictors && 'SELECT_PREDICTORS'}
                                </div>
                                {collapsedWidgets.predictors ? <ChevronDown size={14} /> : <ChevronUp size={14} />}
                            </div>
                            {!collapsedWidgets.predictors && (
